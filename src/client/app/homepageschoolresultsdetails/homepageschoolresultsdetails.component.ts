...
import {homepageschoolresultsdetailsTS} from './homepageschoolresultsdetails';
...
@Component({
  moduleId: module.id,
  selector: 'sd-homepageschoolresultsdetails',
  templateUrl: 'homepageschoolresultsdetails.component.html',
  styleUrls: ['homepageschoolresultsdetails.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class HomepageschoolresultsdetailsComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: homepageschoolresultsdetailsTS;
   selectedRow:  homepageschoolresultsdetailsTS;

   objList:homepageschoolresultsdetailsTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new homepageschoolresultsdetailsTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: HomepageschoolresultsdetailsService,
 ...
 onRowSelecthomepageschoolresultsdetails(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:homepageschoolresultsdetailsTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.HpsrdId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
  this.userform = this.fb.group({'HpsrdName':new FormControl('', Validators. required),'HpsrdSrNo':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),});
  ...
  this.localService
    ...
    () => console.log('done getDisplayAll2' + JSON.stringify(this.objList)));
      ...
      deleteRow(cont: homepageschoolresultsdetailsTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:homepageschoolresultsdetailsTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('homepageschoolresultsdetails','HpsrdId',selectedkeywordgridRow.HpsrdId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:homepageschoolresultsdetailsTS): number {
