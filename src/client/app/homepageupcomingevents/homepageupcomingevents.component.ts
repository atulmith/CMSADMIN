...
import {homepageupcomingeventsTS} from './homepageupcomingevents';
...
@Component({
  moduleId: module.id,
  selector: 'sd-homepageupcomingevents',
  templateUrl: 'homepageupcomingevents.component.html',
  styleUrls: ['homepageupcomingevents.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class HomepageupcomingeventsComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: homepageupcomingeventsTS;
   selectedRow:  homepageupcomingeventsTS;

   objList:homepageupcomingeventsTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new homepageupcomingeventsTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: HomepageupcomingeventsService,
 ...
 onRowSelecthomepageupcomingevents(event: any){

  this.isnewobj=false;
  let temphomepagedescriptionimg:homepageupcomingeventsTS;
  temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

    this.localService.edit(temphomepagedescriptionimg.HpueId).subscribe(
      p => this.objVar=p,
      e => console.log( e), 
      () => console.log("edit") );
...
{ 
    this.clear();
    this.userform = this.fb.group({'HpueTitle':new FormControl('', Validators. required),'HpueDescrption':new FormControl('', Validators. required),'SrNo':new FormControl('', Validators. required),'HpueYear':new FormControl('', Validators. required),});
    ...
    deleteRow(cont: homepageupcomingeventsTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:homepageupcomingeventsTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('homepageupcomingevents','HpueId',selectedkeywordgridRow.HpueId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:homepageupcomingeventsTS): number {
