...
import {achievementsallTS} from './achievementsall';
...
@Component({
  moduleId: module.id,
  selector: 'sd-achievementsall',
  templateUrl: 'achievementsall.component.html',
  styleUrls: ['achievementsall.component.css'],
...
export class AchievementsallComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: achievementsallTS;
   selectedRow:  achievementsallTS;

   objList:achievementsallTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   listoftypeofschool:SelectItem[]=[];


      clear(){
            this.isnewobj=true;
            this.objVar=new achievementsallTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AchievementsallService,
 ...
 onRowSelectachievementsall(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:achievementsallTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.AaId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
this.userform = this.fb.group({'AaTitle':new FormControl('', Validators. required),'AaType':new FormControl('', Validators. required),});
...
e => console.log( e),
 () => console.log('done getDisplayAll2' + JSON.stringify(this.objList)));
  ...
  },
  {
    ...
    gotodetails(obj:achievementsallTS) {
      let aid=obj.AaId;
        this.router.navigate(['achievementsalldetails/'+aid]);
    }
  
    deleteRow(cont: achievementsallTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:achievementsallTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
                        
            let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('achievementsall','AaId',selectedkeywordgridRow.AaId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:achievementsallTS): number {
