...
import {academicsprimarysecondarySchoolTS} from './academicsprimarysecondarySchool';
...
@Component({
  moduleId: module.id,
  selector: 'sd-academicsprimarysecondarySchool',
  templateUrl: 'academicsprimarysecondarySchool.component.html',
  styleUrls: ['academicsprimarysecondarySchool.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class AcademicsprimarysecondarySchoolComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: academicsprimarysecondarySchoolTS;
   selectedRow:  academicsprimarysecondarySchoolTS;

   objList:academicsprimarysecondarySchoolTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new academicsprimarysecondarySchoolTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AcademicsprimarysecondarySchoolService,
 ...
 onRowSelectacademicsprimarysecondarySchool(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:academicsprimarysecondarySchoolTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.ApssId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
    this.userform = this.fb.group({'ApssTitle':new FormControl('', Validators. required),'ApssDescription':new FormControl('', Validators. required),'SrNo':new FormControl('', Validators. required),});
    ...
    deleteRow(cont: academicsprimarysecondarySchoolTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:academicsprimarysecondarySchoolTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
            
            let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('academicsprimarysecondaryschool','ApssId',selectedkeywordgridRow.ApssId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:academicsprimarysecondarySchoolTS): number {
