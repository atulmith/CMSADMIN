...
import {academicsmidssckarateTS} from './academicsmidssckarate';
...
import { AcademicsmidssckarateService } from '../shared/academicsmidssckarate/index';
...
@Component({
  moduleId: module.id,
  selector: 'sd-academicsmidssckarate',
  templateUrl: 'academicsmidssckarate.component.html',
  styleUrls: ['academicsmidssckarate.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class AcademicsmidssckarateComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: academicsmidssckarateTS;
   selectedRow:  academicsmidssckarateTS;

   objList:academicsmidssckarateTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new academicsmidssckarateTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AcademicsmidssckarateService,
 ...
 onRowSelectacademicsmidssckarate(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:academicsmidssckarateTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.AmskId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
ngOnInit() 
  { 
    this.clear();
  this.userform = this.fb.group({'AmskTitle':new FormControl('', Validators. required),'AmskDescription':new FormControl('', Validators. required),'AmskSrNo':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),});
  ...
  () => console.log('done getDisplayAll2' + JSON.stringify(this.objList)));
    ...
    deleteRow(cont: academicsmidssckarateTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:academicsmidssckarateTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

             let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('academicsmidssckarate','AmskId',selectedkeywordgridRow.AmskId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:academicsmidssckarateTS): number {
