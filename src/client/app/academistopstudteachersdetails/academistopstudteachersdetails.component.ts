...
import {academistopstudteachersdetailsTS} from './academistopstudteachersdetails';
...
@Component({
  moduleId: module.id,
  selector: 'sd-academistopstudteachersdetails',
  templateUrl: 'academistopstudteachersdetails.component.html',
  styleUrls: ['academistopstudteachersdetails.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class AcademistopstudteachersdetailsComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: academistopstudteachersdetailsTS;
   selectedRow:  academistopstudteachersdetailsTS;

   objList:academistopstudteachersdetailsTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new academistopstudteachersdetailsTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AcademistopstudteachersdetailsService,
...
onRowSelectacademistopstudteachersdetails(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:academistopstudteachersdetailsTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.AtstdId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
this.userform = this.fb.group({'AtstdSubTitle':new FormControl('', Validators. required),'AtstdSrNo':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),});
...
e => console.log( e),
 () => console.log('done getDisplayAll2' + JSON.stringify(this.objList)));
    ...
    deleteRow(cont: academistopstudteachersdetailsTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:academistopstudteachersdetailsTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;


            let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('academistopstudteachersdetails','AtstdId',selectedkeywordgridRow.AtstdId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:academistopstudteachersdetailsTS): number {
