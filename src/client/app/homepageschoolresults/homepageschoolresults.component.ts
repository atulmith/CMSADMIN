...
import {homepageschoolresultsTS} from './homepageschoolresults';
...
@Component({
  moduleId: module.id,
  selector: 'sd-homepageschoolresults',
  templateUrl: 'homepageschoolresults.component.html',
  styleUrls: ['homepageschoolresults.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class HomepageschoolresultsComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: homepageschoolresultsTS;
   selectedRow:  homepageschoolresultsTS;

   objList:homepageschoolresultsTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new homepageschoolresultsTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: HomepageschoolresultsService,
 ...
 onRowSelecthomepageschoolresults(event: any){

  this.isnewobj=false;
  let temphomepagedescriptionimg:homepageschoolresultsTS;
  temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

  this.localService.edit(temphomepagedescriptionimg.HpsrId).subscribe(
      p => this.objVar=p,
      e => console.log( e), 
      () => console.log("edit") );
...
{ 
  this.clear();
  this.userform = this.fb.group({'HpsrTitle':new FormControl('', Validators. required),'Hpsrdescription':new FormControl('', Validators. required),});
  ...
  this.localService
    ...
    deleteRow(cont: homepageschoolresultsTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:homepageschoolresultsTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('homepageschoolresults','HpsrId',selectedkeywordgridRow.HpsrId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:homepageschoolresultsTS): number {
