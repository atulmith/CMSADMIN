...
import {academistopstudteachersTS} from './academistopstudteachers';
...
@Component({
  moduleId: module.id,
  selector: 'sd-academistopstudteachers',
  templateUrl: 'academistopstudteachers.component.html',
  styleUrls: ['academistopstudteachers.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class AcademistopstudteachersComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: academistopstudteachersTS;
   selectedRow:  academistopstudteachersTS;

   objList:academistopstudteachersTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new academistopstudteachersTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AcademistopstudteachersService,
...
onRowSelectacademistopstudteachers(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:academistopstudteachersTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.AtstId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
ngOnInit() 
{ this.clear();
this.userform = this.fb.group({'AtstTitle':new FormControl('', Validators. required),});
...
e => console.log( e),
 () => console.log('done getDisplayAll2' + JSON.stringify(this.objList)));
    ...
    deleteRow(cont: academistopstudteachersTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:academistopstudteachersTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
             
            let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('academistopstudteachers','AtstId',selectedkeywordgridRow.AtstId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:academistopstudteachersTS): number {
