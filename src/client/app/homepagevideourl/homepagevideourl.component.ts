...
import {homepagevideourlTS} from './homepagevideourl';
...
import { HomepagevideourlService } from '../shared/homepagevideourl/index';
...
@Component({
  moduleId: module.id,
  selector: 'sd-homepagevideourl',
  templateUrl: 'homepagevideourl.component.html',
  styleUrls: ['homepagevideourl.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class HomepagevideourlComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: homepagevideourlTS;
   selectedRow:  homepagevideourlTS;

   objList:homepagevideourlTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new homepagevideourlTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: HomepagevideourlService,
 ...
 onRowSelecthomepagevideourl(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:homepagevideourlTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.HpvurlId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
        this.clear();
      this.userform = this.fb.group({'HpvurlTitle':new FormControl('', Validators. required),'HpvurlLink':new FormControl('', Validators. required),});
    ...
    deleteRow(cont: homepagevideourlTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:homepagevideourlTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('homepagevideourl','HpvurlId',selectedkeywordgridRow.HpvurlId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:homepagevideourlTS): number {
