...
import {specialaidTS} from './specialaid';
...
@Component({
  moduleId: module.id,
  selector: 'sd-specialaid',
  templateUrl: 'specialaid.component.html',
  styleUrls: ['specialaid.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class SpecialaidComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: specialaidTS;
   selectedRow:  specialaidTS;

   objList:specialaidTS[]=[];
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: SpecialaidService,
 ...
 onRowSelectspecialaid(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:specialaidTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.SaId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
  this.userform = this.fb.group({'SaTitle':new FormControl('', Validators. required),'SaDescriptionText':new FormControl('', Validators. required),'SaSrNo':new FormControl('', Validators. required),});
  ...
  this.localService
    ...
    deleteRow(cont: specialaidTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:specialaidTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('specialaid','SaId',selectedkeywordgridRow.SaId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:specialaidTS): number {
