...
import {newsannouncementTS} from './newsannouncement';
...
import { NewsannouncementService } from '../shared/newsannouncement/index';
...
@Component({
  moduleId: module.id,
  selector: 'sd-newsannouncement',
  templateUrl: 'newsannouncement.component.html',
  styleUrls: ['newsannouncement.component.css'],
...
export class NewsannouncementComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: newsannouncementTS;
   selectedRow:  newsannouncementTS;

   objList:newsannouncementTS[]=[];
  ...
  fileupload2: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new newsannouncementTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: NewsannouncementService,
...
onRowSelectnewsannouncement(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:newsannouncementTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.NaId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
  this.userform = this.fb.group({'NaTitle':new FormControl('', Validators. required),'NaSubTitle':new FormControl('', Validators. required),'NaDescription':new FormControl('', Validators. required),'NaSrNo':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),'filename2':new FormControl('', Validators. required),'newslink':new FormControl('', Validators. required),});
  ...
  this.localService
    ...
    deleteRow(cont: newsannouncementTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:newsannouncementTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('newsannouncement','NaId',selectedkeywordgridRow.NaId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:newsannouncementTS): number {
