...
import {academicsschoolactivitiesTS} from './academicsschoolactivities';
...
@Component({
  moduleId: module.id,
  selector: 'sd-academicsschoolactivities',
  templateUrl: 'academicsschoolactivities.component.html',
  styleUrls: ['academicsschoolactivities.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class AcademicsschoolactivitiesComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: academicsschoolactivitiesTS;
   selectedRow:  academicsschoolactivitiesTS;

   objList:academicsschoolactivitiesTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new academicsschoolactivitiesTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AcademicsschoolactivitiesService,
...
onRowSelectacademicsschoolactivities(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:academicsschoolactivitiesTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.AsaId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
ngOnInit() 
{ this.clear();
this.userform = this.fb.group({'AsaTitle':new FormControl('', Validators. required),'AsaDescription':new FormControl('', Validators. required),});
...
e => console.log( e),
 () => console.log('done getDisplayAll2' + JSON.stringify(this.objList)));
    ...
    deleteRow(cont:  academicsschoolactivitiesTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:academicsschoolactivitiesTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
            
            let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('academicsschoolactivities','AsaId',selectedkeywordgridRow.AsaId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:academicsschoolactivitiesTS): number {
