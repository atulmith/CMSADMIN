...
import {academicspriprimaryTS} from './academicspriprimary';
...
@Component({
  moduleId: module.id,
  selector: 'sd-academicspriprimary',
  templateUrl: 'academicspriprimary.component.html',
  styleUrls: ['academicspriprimary.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class AcademicspriprimaryComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: academicspriprimaryTS;
   selectedRow:  academicspriprimaryTS;

   objList:academicspriprimaryTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new academicspriprimaryTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AcademicspriprimaryService,
 ...
 onRowSelectacademicspriprimary(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:academicspriprimaryTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.AcpripId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
  this.userform = this.fb.group({'AcpripTitle':new FormControl('', Validators. required),'AcpripDescription':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),});
  ...
  () => console.log('done getDisplayAll2' + JSON.stringify(this.objList)));
    ...
    deleteRow(cont: academicspriprimaryTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:academicspriprimaryTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
            
 
            let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('academicspriprimary','AcpripId',selectedkeywordgridRow.AcpripId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:academicspriprimaryTS): number {
