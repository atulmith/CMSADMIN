...
import {homepageaddresslocationTS} from './homepageaddresslocation';
...
import { HomepageaddresslocationService } from '../shared/homepageaddresslocation/index';
...
@Component({
  moduleId: module.id,
  selector: 'sd-homepageaddresslocation',
  templateUrl: 'homepageaddresslocation.component.html',
  styleUrls: ['homepageaddresslocation.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class HomepageaddresslocationComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: homepageaddresslocationTS;
   selectedRow:  homepageaddresslocationTS;

   objList:homepageaddresslocationTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new homepageaddresslocationTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: HomepageaddresslocationService,
 ...
 onRowSelecthomepageaddresslocation(event: any){

  this.isnewobj=false;
  let temphomepagedescriptionimg:homepageaddresslocationTS;
  temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

  this.localService.edit(temphomepagedescriptionimg.HpalId).subscribe(
    p => this.objVar=p,
    e => console.log( e), 
    () => console.log("edit") );
...
{ 
  this.clear();
  this.userform = this.fb.group({'HpalTitle':new FormControl('', Validators. required),'HpalSchoolName':new FormControl('', Validators. required),'HpalAddress':new FormControl('', Validators. required),'HpalContactNos':new FormControl('', Validators. required),'HpalEmailid':new FormControl('', Validators. required),'HpalLocationCoordinates':new FormControl('', Validators. required),});
  ...
  this.localService
    ...
    deleteRow(cont: homepageaddresslocationTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:homepageaddresslocationTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
            
 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('homepageaddresslocation','HpalId',selectedkeywordgridRow.HpalId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:homepageaddresslocationTS): number {
