...
import {homepagedescriptionimgTS} from './homepagedescriptionimg';
...
@Component({
  moduleId: module.id,
  selector: 'sd-homepagedescriptionimg',
  templateUrl: 'homepagedescriptionimg.component.html',
  styleUrls: ['homepagedescriptionimg.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class HomepagedescriptionimgComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: homepagedescriptionimgTS;
   selectedRow:  homepagedescriptionimgTS;

   objList:homepagedescriptionimgTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new homepagedescriptionimgTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: HomepagedescriptionimgService,
 ...
 onRowSelecthomepagedescriptionimg(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:homepagedescriptionimgTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.HpdiId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
onUpload(event: any){
  ...
  { 
    this.clear();
    this.userform = this.fb.group({'HpdiTitle':new FormControl('', Validators. required),'HpdiDescription':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),});
//'filename':new FormControl('', Validators. required),'filetype':new FormControl('', Validators. required),'filepath':new FormControl('', Validators. required)
    this.localService
      .getall('DUMMY')
      .subscribe(
      p => this.objList = p,
    ...
    deleteRow(cont: homepagedescriptionimgTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:homepagedescriptionimgTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
            
 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('homepagedescriptionimg','HpdiId',selectedkeywordgridRow.HpdiId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:homepagedescriptionimgTS): number {
