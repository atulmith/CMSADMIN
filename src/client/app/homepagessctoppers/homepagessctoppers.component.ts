...
import {homepagessctoppersTS} from './homepagessctoppers';
...
@Component({
  moduleId: module.id,
  selector: 'sd-homepagessctoppers',
  templateUrl: 'homepagessctoppers.component.html',
  styleUrls: ['homepagessctoppers.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class HomepagessctoppersComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: homepagessctoppersTS;
   selectedRow:  homepagessctoppersTS;

   objList:homepagessctoppersTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new homepagessctoppersTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: HomepagessctoppersService,
 ...
 onRowSelecthomepagessctoppers(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:homepagessctoppersTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.HpstId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
    this.clear();
    this.userform = this.fb.group({'HpstName':new FormControl('', Validators. required),'HpstText':new FormControl('', Validators. required),'HpstSrNo':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),});
    ...
    deleteRow(cont: homepagessctoppersTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:homepagessctoppersTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;

 let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('homepagessctoppers','HpstId',selectedkeywordgridRow.HpstId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:homepagessctoppersTS): number {
