...
import {achivementsgraphsTS} from './achivementsgraphs';
...
@Component({
  moduleId: module.id,
  selector: 'sd-achivementsgraphs',
  templateUrl: 'achivementsgraphs.component.html',
  styleUrls: ['achivementsgraphs.component.css'],
  providers:[MyDateFormat,MyTimeFormat,BaseUrlService,ConfirmationService]
})
export class AchivementsgraphsComponent implements OnInit {
  ...
  msgs: Message[]=[];
   ...
   objVar: achivementsgraphsTS;
   selectedRow:  achivementsgraphsTS;

   objList:achivementsgraphsTS[]=[];
  ...
  fileupload: FileUpload;
   ...
   isnewobj:boolean=false;
      ...
      clear(){
            this.isnewobj=true;
            this.objVar=new achivementsgraphsTS();
  ...
  constructor(private fb: FormBuilder,
              ...
              private localService: AchivementsgraphsService,
 ...
 onRowSelectachivementsgraphs(event: any){

this.isnewobj=false;
 let temphomepagedescriptionimg:achivementsgraphsTS;
 temphomepagedescriptionimg=this.selectedRow;

//this.ngModelHpdiId= temphomepagedescriptionimg.HpdiId;

this.localService.edit(temphomepagedescriptionimg.AgId).subscribe(
p => this.objVar=p,
 e => console.log( e), 
 () => console.log("edit") );
...
{ 
  this.clear();
  this.userform = this.fb.group({'AgTitle':new FormControl('', Validators. required),'AgSrNo':new FormControl('', Validators. required),'filename':new FormControl('', Validators. required),});
  ...
  this.localService
    ...
    deleteRow(cont: achivementsgraphsTS) {
      ...
      this.confirmationService.confirm({
            ...
            let selectedkeywordgridRow:achivementsgraphsTS=cont;
            this.objList.splice(this.findSelectedKeywordSubcatIndex(selectedkeywordgridRow), 1);
            cont = null;
            
            let deleted:boolean=false;
            this.baseUrlservice.callCommonDeleteWS('achivementsgraphs','AgId',selectedkeywordgridRow.AgId)
              .subscribe(
              p => deleted=p,
              e => console.log( e),
              () => {
  ...
  findSelectedKeywordSubcatIndex(p:achivementsgraphsTS): number {
